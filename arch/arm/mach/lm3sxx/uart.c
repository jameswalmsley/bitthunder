...
static BT_ERROR uartGetAvailable(BT_HANDLE hUart, BT_u32 *pTransmit, BT_u32 *pReceive) {
	...
	case BT_UART_MODE_POLLED:
	{
		if((hUart->pRegs->FR & LM3Sxx_UART_FR_TXFE)) *pTransmit = 1;
		if((hUart->pRegs->FR & LM3Sxx_UART_FR_RXFE)) *pReceive = 1;
		break;
	}
...
static BT_s32 file_write(BT_HANDLE hUart, BT_u32 ulFlags, BT_u32 ulSize, const void *pBuffer) {
