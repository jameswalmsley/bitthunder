...
BT_DEF_MODULE_NAME			("LPC11xx-TIMER")
BT_DEF_MODULE_DESCRIPTION	("LPC11xx Timers kernel driver, also providing kernel tick")
...
static BT_u32 timer_getinputclock(BT_HANDLE hTimer, BT_ERROR *pError) {
	...
	*pError = BT_ERR_NONE;
	...
	return BT_LPC11xx_GetSystemFrequency() / (pRegs->TMRBPR+1);
}
...
static BT_ERROR timer_setconfig(BT_HANDLE hTimer, void *pConfig) {
	...
	pRegs->TMRBMR1	= ((BT_LPC11xx_TIMER_CONFIG*)pConfig)->Match[1];
	pRegs->TMRBMR2	= ((BT_LPC11xx_TIMER_CONFIG*)pConfig)->Match[2];
	pRegs->TMRBCCR	= ((BT_LPC11xx_TIMER_CONFIG*)pConfig)->CaptureControl;
	pRegs->TMRBEMR	= ((BT_LPC11xx_TIMER_CONFIG*)pConfig)->ExtMatchControl;
	pRegs->TMRBPWMC	= ((BT_LPC11xx_TIMER_CONFIG*)pConfig)->PWMControl;
...
static BT_ERROR timer_getconfig(BT_HANDLE hTimer, void *pConfig) {
	...
	((BT_LPC11xx_TIMER_CONFIG*)pConfig)->Control			= pRegs->TMRBCTCR;
	((BT_LPC11xx_TIMER_CONFIG*)pConfig)->MatchControl		= pRegs->TMRBMCR;
	((BT_LPC11xx_TIMER_CONFIG*)pConfig)->Match[0]			= pRegs->TMRBMR0;
	((BT_LPC11xx_TIMER_CONFIG*)pConfig)->Match[1]			= pRegs->TMRBMR1;
	((BT_LPC11xx_TIMER_CONFIG*)pConfig)->Match[2]			= pRegs->TMRBMR2;
	((BT_LPC11xx_TIMER_CONFIG*)pConfig)->CaptureControl	= pRegs->TMRBCCR;
	((BT_LPC11xx_TIMER_CONFIG*)pConfig)->ExtMatchControl	= pRegs->TMRBEMR;
	((BT_LPC11xx_TIMER_CONFIG*)pConfig)->PWMControl		= pRegs->TMRBPWMC;
...
static BT_ERROR timer_start(BT_HANDLE hTimer) {
	...
	pRegs->TMRBTCR |= LPC11xx_TIMER_TMRBTCR_CEn;
...
static void enableTimerPeripheralClock(BT_HANDLE hTimer) {
	...
	case 0: {
		LPC11xx_RCC->SYSAHBCLKCTRL |= LPC11xx_RCC_SYSAHBCLKCTRL_TMR0EN;
		break;
	}
	case 1: {
		LPC11xx_RCC->SYSAHBCLKCTRL |= LPC11xx_RCC_SYSAHBCLKCTRL_TMR1EN;
		break;
	}
	case 2: {
		LPC11xx_RCC->SYSAHBCLKCTRL |= LPC11xx_RCC_SYSAHBCLKCTRL_TMR2EN;
		break;
	}
	case 3: {
		LPC11xx_RCC->SYSAHBCLKCTRL |= LPC11xx_RCC_SYSAHBCLKCTRL_TMR3EN;
		break;
	}
	default: {
		break;
...
static BT_BOOL isTimerPeripheralClockEnabled(BT_HANDLE hTimer) {
	...
	switch(pResource->ulStart) {
	case 0: {
		if(LPC11xx_RCC->SYSAHBCLKCTRL & LPC11xx_RCC_SYSAHBCLKCTRL_TMR0EN) {
			return BT_TRUE;
		}
		break;
	}
	case 1: {
		if(LPC11xx_RCC->SYSAHBCLKCTRL & LPC11xx_RCC_SYSAHBCLKCTRL_TMR1EN) {
			return BT_TRUE;
		}
		break;
	}
	case 2: {
		if(LPC11xx_RCC->SYSAHBCLKCTRL & LPC11xx_RCC_SYSAHBCLKCTRL_TMR2EN) {
			return BT_TRUE;
		}
		break;
	}
	case 3: {
		if(LPC11xx_RCC->SYSAHBCLKCTRL & LPC11xx_RCC_SYSAHBCLKCTRL_TMR3EN) {
			return BT_TRUE;
		}
		break;
	}
...
static const BT_IF_HANDLE oHandleInterface = {
	BT_MODULE_DEF_INFO,
	.oIfs = {
		(BT_HANDLE_INTERFACE) &BT_LPC11xx_TIMER_oDeviceInterface,
	},
...
static BT_HANDLE timer_probe(const BT_INTEGRATED_DEVICE *pDevice, BT_ERROR *pError) {
	...
	hTimer->pRegs = (LPC11xx_TIMER_REGS *) pResource->ulStart;
...
#ifdef BT_CONFIG_MACH_LPC11xx_TIMER_0
static const BT_RESOURCE oLPC11xx_timer0_resources[] = {
	{
		.ulStart 			= BT_CONFIG_MACH_LPC11xx_TIMER0_BASE,
		.ulEnd 				= BT_CONFIG_MACH_LPC11xx_TIMER0_BASE + BT_SIZE_4K - 1,
...
const BT_DEVFS_INODE_DEF oLPC11xx_timer0_inode = {
	.szpName = "timer0",
	.pDevice = &oLPC11xx_timer0_device,
};
#endif

#ifdef BT_CONFIG_MACH_LPC11xx_TIMER_1
static const BT_RESOURCE oLPC11xx_timer1_resources[] = {
	{
		.ulStart 			= BT_CONFIG_MACH_LPC11xx_TIMER1_BASE,
		.ulEnd 				= BT_CONFIG_MACH_LPC11xx_TIMER1_BASE + BT_SIZE_4K - 1,
		.ulFlags 			= BT_RESOURCE_MEM,
	},
	{
		.ulStart			= 1,
...
const BT_DEVFS_INODE_DEF oLPC11xx_timer1_inode = {
	.szpName = "timer1",
	.pDevice = &oLPC11xx_timer1_device,
};
#endif

#ifdef BT_CONFIG_MACH_LPC11xx_TIMER_2
static const BT_RESOURCE oLPC11xx_timer2_resources[] = {
	{
		.ulStart 			= BT_CONFIG_MACH_LPC11xx_TIMER2_BASE,
		.ulEnd 				= BT_CONFIG_MACH_LPC11xx_TIMER2_BASE + BT_SIZE_4K - 1,
		.ulFlags 			= BT_RESOURCE_MEM,
	},
	{
		.ulStart			= 2,
...
const BT_DEVFS_INODE_DEF oLPC11xx_timer2_inode = {
	.szpName = "timer2",
	.pDevice = &oLPC11xx_timer2_device,
};
#endif

#ifdef BT_CONFIG_MACH_LPC11xx_TIMER_3
static const BT_RESOURCE oLPC11xx_timer3_resources[] = {
	{
		.ulStart 			= BT_CONFIG_MACH_LPC11xx_TIMER3_BASE,
		.ulEnd 				= BT_CONFIG_MACH_LPC11xx_TIMER3_BASE + BT_SIZE_4K - 1,
		.ulFlags 			= BT_RESOURCE_MEM,
	},
	{
		.ulStart			= 3,
...
const BT_DEVFS_INODE_DEF oLPC11xx_timer3_inode = {
	.szpName = "timer3",
	.pDevice = &oLPC11xx_timer3_device,
};
#endif
