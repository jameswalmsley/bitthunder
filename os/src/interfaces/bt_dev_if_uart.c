...
static BT_BOOL isUartHandle(BT_HANDLE hUart) {
	if(!hUart || !hUart->h.pIf->pIfs->pDevIF || (hUart->h.pIf->pIfs->pDevIF->eConfigType != BT_DEV_IF_T_UART)) {
		return BT_FALSE;
	}
	return BT_TRUE;
}
...
BT_ERROR BT_UartSetBaudrate(BT_HANDLE hUart, BT_u32 ulBaudrate) {
	...
	return hUart->h.pIf->pIfs->pDevIF->unConfigIfs->pUartIF->pfnSetBaudrate(hUart, ulBaudrate);
}
...
BT_ERROR BT_UartSetConfiguration(BT_HANDLE hUart, BT_UART_CONFIG *pConfig) {
	...
	return hUart->h.pIf->pIfs->pDevIF->unConfigIfs->pUartIF->pfnSetConfig(hUart, pConfig);
}
...
BT_ERROR BT_UartGetConfiguration(BT_HANDLE hUart, BT_UART_CONFIG *pConfig) {
	...
	return hUart->h.pIf->pIfs->pDevIF->unConfigIfs->pUartIF->pfnGetConfig(hUart, pConfig);
}
...
BT_ERROR BT_UartEnable(BT_HANDLE hUart) {
	...
	return hUart->h.pIf->pIfs->pDevIF->unConfigIfs->pUartIF->pfnEnable(hUart);
}
...
BT_ERROR BT_UartDisable(BT_HANDLE hUart) {
	...
	return hUart->h.pIf->pIfs->pDevIF->unConfigIfs->pUartIF->pfnDisable(hUart);
}
