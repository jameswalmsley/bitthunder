...
typedef struct _BT_IF_FS {
	...
	BT_ERROR	(*pfnMkDir)		(BT_HANDLE hMount, BT_i8 *szpPath);
	BT_HANDLE	(*pfnOpenDir)	(BT_HANDLE hMount, BT_i8 *szpPath, BT_ERROR *pError);
	BT_HANDLE	(*pfnGetInode)	(BT_HANDLE hMount, BT_i8 *szpPath, BT_ERROR *pError);
} BT_IF_FS;
