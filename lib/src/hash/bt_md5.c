...
*/
/*
 *	Adapted for BitThunder by James Walmsley to be consistent with the BT hashing apis.
 *	An unmodified version of the md5 sources is available under:
 *	scripts/mkimage/md5.[c|h]
 */
...
#  define BYTE_ORDER 1
#elif defined(BT_CONFIG_LITTLE_ENDIAN)
#  define BYTE_ORDER -1
#else
#  define BYTE_ORDER 0
#endif

#define T_MASK ((BT_u32)~0)
...
static void
md5_process(struct bt_md5_context *ctx, const BT_u8 *data /*[64]*/)
{
    BT_u32
...

    {
		...
		if (*((const BT_u8 *)&w)) /* dynamic little-endian */
#endif
...
void bt_md5_init(struct bt_md5_context *ctx) {
...
void bt_md5_finish(struct bt_md5_context *ctx, BT_u8 digest[16]) {
    ...
    for (i = 0; i < 8; ++i)
		data[i] = (BT_u8)(ctx->count[i >> 2] >> ((i & 3) << 3));
    /* Pad to 56 bytes mod 64. */
    bt_md5_append(ctx, pad, ((55 - (ctx->count[0] >> 3)) & 63) + 1);
    /* Append the length. */
    bt_md5_append(ctx, data, 8);
    for (i = 0; i < 16; ++i)
